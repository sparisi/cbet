...
def learn(actor_model,
    ...
    with lock:
        ...
        count_rewards = batch['reset_state_count'][1:].float().to(device=flags.device)
...
def train(flags):
    ...
    forward_dynamics_model = models_and_states['forward_dynamics_model']
    ...
    forward_dynamics_optimizer = models_and_states['forward_dynamics_optimizer']
    ...
    def batch_and_learn(i, lock=threading.Lock()):
        ...
        while frames < flags.total_frames:
            ...
            stats = learn(actor_model, learner_model, learner_exploration_model,
                          state_embedding_model, forward_dynamics_model, inverse_dynamics_model,
                          batch, agent_state, exploration_agent_state,
                          learner_model_optimizer,
                          state_embedding_optimizer, forward_dynamics_optimizer, inverse_dynamics_optimizer,
                          scheduler, flags)
            timings.time('learn')
            with lock:
